...
`pnpm run doctor` はPATH上のローカルAI agent CLIを確認します。Codexが見つかれば推奨として表示しますが、Codex専用ではなく、他のローカルagentでも同じプロジェクトJSONとCLIを使えます。

## 多言語対応

初期UIは `ja-JP`, `en`, `zh-CN`, `zh-TW`, `ko`, `es`, `fr`, `de`, `pt-BR`, `it`, `ru`, `id`, `hi` の13言語をサポートします。詳しくは [docs/I18N.md](docs/I18N.md) を見てください。
